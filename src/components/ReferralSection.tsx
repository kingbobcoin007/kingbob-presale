...
import { REFERRAL_REWARD, calculateTokenPrice } from '../utils/ethersHelpers';
...
const ReferralSection: React.FC<ReferralSectionProps> = ({ userAddress }) => {
  ...
  // Calculate token value in USD based on the token price
    ...
    const fetchReferralData = async () => {
      ...
      try {
        ...
        if (userSnap.exists()) {
          ...
          // Get actual referral count from Firestore
          const referralsQuery = query(
            collection(db, usersCollection),
            where('referrer', '==', userAddress)
          );
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="bg-white rounded-lg p-4 shadow">
                  ...
                  <p className="text-sm text-gray-500">≈ ${calculateTokenValue(referralRewards)} USD</p>
                </div>
                <div className="bg-white rounded-lg p-4 shadow">
                  ...
                  <p className="text-sm text-gray-500">Earn {REFERRAL_REWARD} tokens per referral</p>
                </div>
              </div>
